function [] = plot_magnitude_spectrum_fft_of(fft_values, input_name)
    ...
    title('FFT Magnitude of Adenine Nucleotide');
    legend('FFT_A(f)');
    grid minor;

    nexttile(t);
    plot(k, abs(fft_values(:,2)), 'color','#A2142F');
    title('FFT Magnitude of Thymine Nucleotide');
    legend('FFT_T(f)');
    grid minor;

    nexttile(t);
    plot(k, abs(fft_values(:,3)), 'color', '#D95319');
    title('FFT Magnitude of Guanine Nucleotide');
    legend('FFT_G(f)');
    grid minor;

    nexttile(t);
    plot(k, abs(fft_values(:,4)), 'color', '#7E2F8E');
    title('FFT Magnitude of Cytosine Nucleotide');
    legend('FFT_C(f)');
    grid minor;
